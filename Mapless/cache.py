...
class Landmark:
    def __init__(self, args):
        ...
        self.le = args.le
        self.landmark_num = args.landmark_num
        self.cache_root = args.cache_root
        if not os.path.exists(args.cache_root + '/landmark.npy'):
            self.initial()
        self.landmarks = np.load(args.cache_root + '/landmark.npy', allow_pickle=True)
        ...
        self.length = len(self.landmarks)
        self.landmarks = torch.tensor(self.landmarks, device=args.device)
    ...
    def scale(self):
        ind = [i for i in range(0, len(self.infos), 5)]
        samples = torch.tensor(self.infos[ind, 1:3].astype("float32"), device=self.device)
        dis = self.land_dis(samples)
        dis = torch.exp(dis * -0.002)
        self.mean_s = dis.mean()
        self.std_s = dis.std()
    ...
    def initial(self):
        ...
        coor_ = infos[1:, :3]#.astype('float32')
        if not le == None:
            coor = coor_[(coor_[:, 1] > le[0]) & (coor_[:, 1] < le[1])]
            coor = coor[(coor[:, 2] > le[2]) & (coor[:, 2] < le[3])]
            le_index = np.isin(infos[:,0], coor[:, 0])
            np.save(self.cache_root + '/area_index.npy', le_index)
        lands_id = np.random.choice(len(coor), size=self.landmark_num, replace=False)
        lands = coor[lands_id]
        lands = lands[:, 1:].astype('float32')
        np.save(self.cache_root + '/landmark.npy', lands)
        #self.visio(coor[:, 1:], lands)

    def visio(self, map, lands):
        plt.scatter(map[:, 0], map[:, 1], c="grey", alpha=0.7)
        plt.scatter(lands[:, 0], lands[:, 1], c="red", s=5)
        plt.show()
        return

    def land_dis(self, nodes):
        B = nodes.shape[0]
        coo = nodes.unsqueeze(1).repeat(1, self.length, 1)
        lands = self.landmarks.unsqueeze(0).repeat(B, 1, 1)
        dis = self.cal.distance(torch.cat((lands, coo), axis=-1)).permute(1, 0)
        return dis


    def embbeding(self, nodes):
        dis = self.land_dis(nodes)
        emb = torch.exp(dis * -0.002)
        emb = (emb - self.mean_s) / self.std_s
...
def Cache_infons(args):
    ...
    if not os.path.exists(args.save_dir + "/pic"):
        os.makedirs(args.save_dir + "/pic")
...
if __name__ == '__main__':
    ...
    goal = np.array([[-73.97004, 40.748875]])
    lander = Landmark(args)
    lander.initial()
    lander.embbeding(goal)
    print(';')
