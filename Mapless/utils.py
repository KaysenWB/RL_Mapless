...
def Get_circle(centers, radius=100, num_points=36, device = 'cpu'):
    ...
    lon = centers[:, :1].repeat(1,num_points) + diff[0] * cos
    lat = centers[:, 1:].repeat(1,num_points) + diff[1] * sin
...
class Landmark:
    ...
    def initial(self):
        ...
        #Visio(coor_[:, 1:], coor[:, 1:], lands, save= self.cache_root + "/pic/landmark.jpg")

    def embbeding(self, nodes):

        coo = np.repeat(nodes, self.length, axis=0)
        coor = np.concatenate((self.landmarks, coo), axis=1)
        dis= self.cal.distance(coor)
        emb = np.exp(dis * -0.002)
        emb = emb / emb.sum()
        return emb
